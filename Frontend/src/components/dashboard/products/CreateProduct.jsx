...
function CreateProduct() {
  ...
  const createMutation = useMutation((data) => createProduct(data, userToken), {
    onSuccess: (data) => {
      if (data.error) {
  ...
  if (catError || !categories || subError) {
    return (
  ...
  return (
    <section className="position-relative">
      {createMutation.isLoading && (
        <div className="loader-wrapper vh-100 bg-transparent ">
          <OverlayLoading />
        </div>
      )}
      <Container fluid>
        <h2 className="border-bottom pb-2 mt-2">إضافة منتج جديد</h2>
