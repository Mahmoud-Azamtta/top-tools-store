...
function UpdateCouponDialog() {
  ...
  const { data: couponData } = useOutletContext();
  ...
  const mutation = useMutation(
    (updatedCoupon) => updateCoupon(id, userToken, updatedCoupon),
    {
      onSuccess: (data) => {
        if (data.error) {
          toast.error(data.error);
        } else {
          toast.success("تم تعديل معلوما الكوبون");
  ...
  const formik = useFormik({
    initialValues: {
      ...
      amount: couponData ? couponData.amount : "",
    ...
    >
      ...
      <Modal.Body>
        {couponData ? (
          ...
          <p className="text-secondary">خطأ اثناء استرجاع بيانات الكوبون...</p>
        )}
      </Modal.Body>
